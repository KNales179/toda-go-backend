...
function windowBounds(window = "7d") {
  ...
  if (window === "today") {
    ...
    start = new Date(
      new Date(localNow).toLocaleString("en-US", { timeZone: "UTC" })
    );
  } else if (window === "7d") {
    start = new Date(end.getTime() - 7 * 86400 * 1000);
  } else if (window === "30d") {
    start = new Date(end.getTime() - 30 * 86400 * 1000);
  } else {
    start = new Date(0);
  }
  return { start, end };
...
router.get("/driver/:driverId/summary", async (req, res) => {
  try {
    ...
    const daily = await Booking.aggregate([
      ...
      { $match: matchCompleted(driverId, start, end, isOverall) },
      {
        $group: {
          _id: {
            day: {
    ...
    const kpiAgg = await Booking.aggregate([
      ...
      },
      {
        $project: {
          ...
          income: 1,
          avgFare: {
            $cond: [{ $gt: ["$trips", 0] }, { $divide: ["$income", "$trips"] }, 0],
          },
          cashIncome: 1,
          gcashIncome: 1,
          distanceKm: 1,
